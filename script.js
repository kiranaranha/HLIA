...
function start() {
    var selectStart = "Select starting place: <select id = startingPlaceSelect>";
    console.log(places[0].name);
    for(var i = 0;  i <= places.length; i++){
        console.log(places[i].name);
        selectStart += places[i].name;
    }

    selectStart += "</select>";
    document.getElementById("startingPlaceDiv").innerHTML = selectStart;
}
